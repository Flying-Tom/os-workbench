...
void coroutine_switch(struct co *co)
{
    ...
    {
    case CO_NEW:
        //stack_switch_call(((((uintptr_t)co->stack + STACK_SIZE) >> 4) << 4), coroutine_entry, (uintptr_t)co);
        stack_switch_call((uintptr_t)((((uintptr_t)co->stack + STACK_SIZE) >> 4) << 4), coroutine_entry, (uintptr_t)co);
        puts("out");
        break;
    case CO_RUNNING:
        //puts("longjmp");
